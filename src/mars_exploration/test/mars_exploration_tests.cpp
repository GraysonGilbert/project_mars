// test_mars_exploration_gtest.cpp
...
TEST(MarsExplorationTest, ClearGoal) {
  ...
  grid.data[0] = 0; // Make one cell free so unknowns can be adjacent to a frontier
  explorer.setMap(grid);
  Pose2D pose{0.0, 0.0, 0.0};
  explorer.updatePose(pose);
  explorer.setMinFrontierDistance(0.0);
...
TEST(MarsExplorationTest, LowConfidenceSettings) {
    ...
    EXPECT_TRUE(explorer.setNearestUnmappedCellAsGoal());
    explorer.clearGoal();
    // Now set low_confidence_value_ lower so cell[1] is not considered low confidence
    // Directly access private member for test (if allowed), otherwise skip
    // For demonstration, assume we can set via a setter (not present in class)
    // explorer.setLowConfidenceValue(5); // If such setter exists
    // Instead, test with a cell value above default threshold
